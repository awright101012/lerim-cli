...
- `activity.log` — append-only activity log (one line per sync/maintain cycle with project, stats, LLM cost, duration)
...
- Per-run LLM cost is tracked automatically via OpenRouter's `usage.cost` response field. PydanticAI calls use an httpx response hook; DSPy calls are captured from LM history. Cost (USD) is logged in `activity.log` and returned in sync/maintain/ask result payloads.
